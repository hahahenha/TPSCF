...
TRIPINFO_FILE = "./data/" + ss + "-tripinfo.xml"
QLENGTH_FILE = "./data/qlengths.xml"
DATAPOINTS_BASE = './logs/datapoints/'

DEADLOCK_THRESH = 2000
...
def record_metrics_of_run(skip_time):
    ...
    while lineno < nlines:
        ...
        if arrival < skip_time:             # discard line
            ...
            timeloss = element.get('timeLoss')
            waitingTime = element.get('waitingTime')
            if waitingTime is None:
                waitingTime = element.get('waitSteps')
            timeLoss_arr_temp.insert(i, float(timeloss) / duration)
            waitingTime_arr_temp.insert(i, float(waitingTime)/duration)
            i += 1
            lineno += 1
