...
class DQN():
    """docstring for DQN"""
    def __init__(self, NUM_STATES, NUM_ACTIONS, MEMORY_CAPACITY, BATCH_SIZE, EPISILO = 0.9, GAMMA = 0.9, Q_NETWORK_ITERATION = 100, LR = 0.01, ENV_A_SHAPE = 0):
        super(DQN, self).__init__()
        self.eval_net, self.target_net = Net(NUM_ACTIONS, NUM_STATES), Net(NUM_ACTIONS, NUM_STATES)
    ...
    def choose_action(self, state, a_space):
        ...
        if action > len(a_space):
            action = action % len(a_space) + a_space[0]
        else:
            action += a_space[0]
...
def main():
    ...
    dqn = DQN(NUM_STATES, NUM_ACTIONS, MEMORY_CAPACITY, BATCH_SIZE, ENV_A_SHAPE=ENV_A_SHAPE)
    episodes = 400
